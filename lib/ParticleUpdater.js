...
class ParticleUpdater {
  ...
  componentUpdater (component) {
    ...
    const buffer = ParticleRenderBuffer.get(component.$node)
    const DOMTree = {}
    const element = createParticleElementMark(curElement, buffer.mark, function (element, id) {
      DOMTree[id] = element
    })

    ParticleElementUpdater.updater(buffer.element, element, component.$node)
    ParticleRenderBuffer.update(component.$node, { element, DOMTree })
    return this
  }
  ...
  componentRender (component, mountNode) {
    ...
    particleEventListener(mountNode, DOMTree)

    // cache component
    component.$node = mountNode
    ParticleRenderBuffer.add(mountNode, { mark, component, element, DOMTree })
