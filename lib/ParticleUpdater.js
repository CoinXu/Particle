...
import ParticleDOM from './ParticleDOM'
...
class ParticleUpdater {
  ...
  componentUpdater (component) {
    ...
    const prevDOMTree = createParticleElementDOMTree(buffer.DOMTree, buffer.mark)
    const curDOMTree = createParticleElementDOMTree(DOMTree, buffer.mark)
    const diff = compareParticleElementDOMTree(prevDOMTree, curDOMTree)

    ParticleDOM.updater(diff, node)
    ParticleRenderBuffer.update(node, { DOMTree })
